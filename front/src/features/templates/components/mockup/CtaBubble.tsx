...
export function CtaBubble({ template }: CtaBubbleProps) {
      ...
      <div className="flex flex-col gap-1 w-[90%] mt-1">
        {actions.length === 0 ? (
          <div className="text-[10px] text-gray-400 dark:text-slate-500 italic px-2">
            Template sem ações configuradas
          </div>
        ) : (
          actions.map((action, idx) => (
            <ActionButton key={idx} icon={iconForAction(action)}>
              {defaultLabelForAction(action)}
            </ActionButton>
          ))
        )}
      </div>
